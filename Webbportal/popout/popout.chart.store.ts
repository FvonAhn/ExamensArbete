...
import { DEFAULT_TOOLTIP_OPACITY, normalizeTooltipOpacity } from "$lib/chart-tooltip";

export interface PopoutChartSnapshot {
  labels: string[];
...
type ChartPayload = Partial<PopoutChartSnapshot>;
type ChartSessionPayload = {
  token?: string | null;
...
export const enabledSeries = writable<string[]>([]);
export const seriesColors = writable<Record<string, string>>({});
export const chartSnapshot = writable<PopoutChartSnapshot>({
  labels: [],
  seriesHistory: {},
  parameterMeta: {},
  enabledSeries: [],
...
let currentSnapshot: PopoutChartSnapshot = {
  labels: [],
  seriesHistory: {},
  parameterMeta: {},
  enabledSeries: [],
...
function applySnapshot(snapshot: PopoutChartSnapshot): void {
  currentSnapshot = snapshot;
  chartSnapshot.set(snapshot);
...
function mergeUiPayload(payload: ChartPayload): void {
  if (!hasDirectFrames) {
    applySnapshot({
      ...
      windowSeconds:
        typeof payload.windowSeconds === "number" && Number.isFinite(payload.windowSeconds)
          ? payload.windowSeconds
          : DEFAULT_CHART_WINDOW_SECONDS,
      highlightedSeriesKey:
        typeof payload.highlightedSeriesKey === "string" ? payload.highlightedSeriesKey : null,
      tooltipOpacity:
        typeof payload.tooltipOpacity === "number" && Number.isFinite(payload.tooltipOpacity)
          ? normalizeTooltipOpacity(payload.tooltipOpacity)
  ...
  applySnapshot({
    ...
    windowSeconds:
      typeof payload.windowSeconds === "number" && Number.isFinite(payload.windowSeconds)
        ? payload.windowSeconds
        : currentSnapshot.windowSeconds,
    highlightedSeriesKey:
      payload.highlightedSeriesKey === null || typeof payload.highlightedSeriesKey === "string"
        ? payload.highlightedSeriesKey
        : currentSnapshot.highlightedSeriesKey,
    tooltipOpacity:
      typeof payload.tooltipOpacity === "number" && Number.isFinite(payload.tooltipOpacity)
        ? normalizeTooltipOpacity(payload.tooltipOpacity)
        : currentSnapshot.tooltipOpacity,
  });
}
...
function appendFrameToSnapshot(frame: TelemetryLiveFrame): void {
  ...
  if (nextLabels.length > MAX_RETAINED_POINTS) nextLabels.shift();
  ...
  applySnapshot({
    ...
    windowSeconds: currentSnapshot.windowSeconds,
    highlightedSeriesKey: currentSnapshot.highlightedSeriesKey,
    tooltipOpacity: currentSnapshot.tooltipOpacity,
  });
}
...
async function stopDirectConnection(): Promise<void> {
  ...
  try {
    ...
    console.warn("[ChartPopout] Failed to stop direct connection", err);
  } finally {
...
async function ensureDirectConnection(session: ChartSessionPayload): Promise<void> {
  ...
  connection.onreconnected(async () => {
    try {
      ...
      console.warn("[ChartPopout] Failed to rejoin device group", err);
    }
  });
  ...
  try {
    ...
    console.warn("[ChartPopout] Failed to start direct connection", err);
    await stopDirectConnection();
  }
}

if (typeof window !== "undefined" && window.opener) {
  window.opener.postMessage({ type: "chart:ready" }, window.location.origin);
}

if (typeof window !== "undefined") {
  window.addEventListener("message", (e) => {
    if (e.data?.type === "chart:session") {
      void ensureDirectConnection((e.data?.payload ?? {}) as ChartSessionPayload);
      return;
    }

    if (e.data?.type === "chart:payload") {
      mergeUiPayload((e.data?.payload ?? {}) as ChartPayload);
    }
  });
